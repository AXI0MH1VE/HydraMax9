...
import { EntropyMetrics } from "../../types";

// Context for operator controls (to be provided by Dashboard)
const OperatorContext = React.createContext<{ entropyThreshold: number }>({ entropyThreshold: 0.1 });

const OverviewView: React.FC = () => {
  const { entropyThreshold } = React.useContext(OperatorContext);
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-full">
      ...
      <div className="lg:col-span-4 flex flex-col gap-6">
        ...
        <section className="h-[280px] border border-[#06af6e]/30 bg-black/40 p-4 brutalist-border overflow-hidden">
          ...
          <SecurityModule entropyThreshold={entropyThreshold} />
        </section>
      </div>
    </div>
  );
